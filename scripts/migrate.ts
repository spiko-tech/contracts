...
import { MigrationManager } from "@amxx/hre/scripts";
import { toMask, combine } from "../test/helpers";
import DEFAULT from "./config.json";
  ...
  .setAction(async (_args, hre) => {
    try {
      const constructorArguments = [
        _args.permissionManagerAddress,
        _args.forwarderAddress,
      ].filter((add) => add !== "");
      ...
      const host =
        _args.chain === "mainnet" || _args.chain === "sepolia"
          ? "etherscan"
          : `polygonscan`;
...
async function verifyContracts(contracts, chainName) {
  ...
  const tokenAddresses = Object.values(contracts.tokens).map(
    (token) => token.target
  );
...
const sanityCheckConfig = (config) => {
  ...
  const rolesSanity = Object.keys(Role).map((roleKey) => {
    ...
    const membersCorrectness = config.roles[roleValue].members.map((member) => {
      const isPermissionedContract = Object.values(
    ...
    return membersCorrectness.every(
      (memberCorrectness) => memberCorrectness === true
    );
  });
...
export type Contracts = {
  ...
  oracles: Record<string, Contract>;
};

export async function migrate(config = {}, opts = {}) {
  ...
  contracts.forwarder = await ethers
    ...
    .then((factory) =>
      migration.migrate("forwarder", factory, ["Forwarder"], { ...opts })
    );
  ...
  const getAddresses = (name) =>
    ethers.isAddress(name)
      ? [ethers.getAddress(name)]
      : getContractByName(name).map(({ target }) => target);
  ...
  const IDS = Object.assign(
    Object.fromEntries(ROLES.map((role, i) => [role, i])),
    { admin: 0, ADMIN: 0, public: 255, PUBLIC: 255 }
  );
  const MASKS = Object.assign(
    Object.fromEntries(ROLES.map((role, i) => [role, toMask(i)])),
    {
      admin: toMask(0),
      ADMIN: toMask(0),
      public: toMask(255),
  ...
  // CHECKS
  expect(["admin", "ADMIN"]).to.include(
    ROLES[0],
    "First role must be admin or ADMIN"
  );
  ...
  const membershipOps = Object.entries(config.roles).flatMap(
    ([role, { members = [] }]) =>
      members
  ...
  const admins = Object.values(config.roles)[0]
    .members.flatMap(getAddresses)
    .unique();
  ...
  allOps.forEach(({ fn, args }) =>
    debug(`- ${fn}(${args.map(JSON.stringify).join(", ")})`)
  );

  await contracts.manager
    .multicall(
      allOps.map(({ fn, args }) =>
        contracts.manager.interface.encodeFunctionData(fn, args)
      )
    )
    .then((txPromise) => txPromise.wait())
    .then(({ logs }) => {
      debug("Events:");
      logs.forEach(({ eventName, args }) =>
