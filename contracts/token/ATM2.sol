...
contract ATM2 is ATM
{
    ...
    function previewSell(IERC20 input, uint256 inputAmount) public view virtual override returns (uint256 tokenAmount, uint256 stableAmount) {
        int256 price = _getPreviousPrice();
        if (input == token) {
    ...
    function _getLastPrice() internal view virtual returns (int256) {
        (, int256 answer,,,) = oracle.latestRoundData();
        return answer;
    }

    function _getPreviousPrice() internal view virtual returns (int256) {
        (uint80 roundId,,,,) = oracle.latestRoundData();
        (, int256 answer,,,) = oracle.getRoundData(roundId - 1);
        return answer;
    }
}
