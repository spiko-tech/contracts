...
async function fixture() {
  ...
  // build rebasing
  contracts.rebasing = await ethers
    .getContractFactory('TokenRebasing')
    .then((factory) =>
      migration.migrate(`token-${tokenConfig.symbol}-rebasing`, factory, [contracts.oracle.target], {
        ...opts,
        kind: 'uups',
...
describe('Main', function () {
  ...
  it('accounts have permissions', async function () {
    ...
    expect(await this.contracts.manager.getGroups(this.accounts.admin)).to.equal(
      combine(this.MASKS.public, this.MASKS.admin)
    );
  ...
  describe('Rebasing Token', function () {
    ...
    describe('EIP721', function () {
      ...
      it('domain is correct', async function () {
        expect(await getDomain(this.contracts.rebasing)).to.deep.equal(this.expectedDomain);
      });
    ...
    describe('ERC20', function () {
      beforeEach(async function () {
        this.setPrice = (price) =>
          Promise.all([time.latest(), this.contracts.oracle.decimals()]).then(([timepoint, decimals]) =>
            this.contracts.oracle
              .connect(this.accounts.operator)
              .publishPrice(timepoint, ethers.parseUnits(price, decimals))
          );
        ...
        await this.contracts.rebasing
          ...
          .deposit(ethers.parseEther('10'), this.accounts.alice);
      });
              ...
              for (const toAuthorized of [true, false]) {
                it(
                  ...
                  async function () {
                    ...
                    // set approval if needed + configure sender and receiver
                    operator && (await this.contracts.rebasing.connect(from).approve(operator, amount));
                    await this.contracts.manager
                      .connect(this.accounts.whitelister)
                      [fromAuthorized ? 'addGroup' : 'remGroup'](from, this.IDS.whitelisted);
                    await this.contracts.manager
                    ...
                    switch (fn) {
                      case 'transfer':
                        promise = this.contracts.rebasing.connect(from).transfer(to, amount);
                        break;
                      case 'transferFrom':
                        promise = this.contracts.rebasing.connect(operator).transferFrom(from, to, amount);
                        break;
                      case 'transferAndCall':
                        promise = this.contracts.rebasing.connect(from).getFunction('transferAndCall(address,uint256)')(
                          to,
                          amount
                        );
                        break;
                    ...
                    await (fromAuthorized && toAuthorized
                      ? expect(promise).to.emit(this.contracts.rebasing, 'Transfer').withArgs(from, to, underlying)
                      : expect(promise)
                          .to.be.revertedWithCustomError(
                            this.contracts.rebasing,
                            (!fromAuthorized && 'UnauthorizedFrom') || (!toAuthorized && 'UnauthorizedTo')
                          )
                          .withArgs(this.contracts.rebasing, (!fromAuthorized && from) || (!toAuthorized && to)));
                  }
                );
              }
          });
      ...
      describe('allowance', function () {
        it('consume correct value', async function () {
          ...
          await expect(
            this.contracts.rebasing
              .connect(this.accounts.chris)
              .transferFrom(this.accounts.alice, this.accounts.bruce, 100n)
          )
            .to.emit(this.contracts.rebasing, 'Transfer')
            .withArgs(this.accounts.alice, this.accounts.bruce, 67n);
      ...
      describe('ERC4626', function () {
        it('deposit', async function () {
          ...
          await expect(this.contracts.rebasing.connect(this.accounts.alice).deposit(assets, this.accounts.bruce))
            .to.emit(this.contracts.token, 'Transfer')
            .withArgs(this.accounts.alice, this.contracts.rebasing, assets)
            .to.emit(this.contracts.rebasing, 'Transfer')
            .withArgs(ethers.ZeroAddress, this.accounts.bruce, assets)
            .to.emit(this.contracts.rebasing, 'Deposit')
            .withArgs(this.accounts.alice, this.accounts.bruce, assets, shares);
        ...
        it('deposit (ERC1363 transferAndCall without data)', async function () {
          ...
          expect(await this.contracts.rebasing.previewDeposit(assets)).to.be.equal(shares);
          await expect(
            this.contracts.token.connect(this.accounts.alice).transferAndCall(this.contracts.rebasing, assets)
          )
            .to.emit(this.contracts.token, 'Transfer')
            .withArgs(this.accounts.alice, this.contracts.rebasing, assets)
            .to.emit(this.contracts.rebasing, 'Transfer')
            .withArgs(ethers.ZeroAddress, this.accounts.alice, assets)
            .to.emit(this.contracts.rebasing, 'Deposit')
            .withArgs(this.accounts.alice, this.accounts.alice, assets, shares);
        ...
        it('deposit (ERC1363 transferAndCall with data)', async function () {
          ...
          await expect(
            this.contracts.token
              .connect(this.accounts.alice)
              .transferAndCall(this.contracts.rebasing, assets, ethers.Typed.bytes(data))
          )
            .to.emit(this.contracts.token, 'Transfer')
            .withArgs(this.accounts.alice, this.contracts.rebasing, assets)
            .to.emit(this.contracts.rebasing, 'Transfer')
        ...
        it('withdraw', async function () {
          ...
          await expect(
            this.contracts.rebasing
              .connect(this.accounts.alice)
              .withdraw(assets, this.accounts.bruce, this.accounts.alice)
          )
            .to.emit(this.contracts.rebasing, 'Transfer')
            .withArgs(this.accounts.alice, ethers.ZeroAddress, assets)
            .to.emit(this.contracts.rebasing, 'Withdraw')
            .withArgs(this.accounts.alice, this.accounts.bruce, this.accounts.alice, assets, shares)
            .to.emit(this.contracts.token, 'Transfer')
            .withArgs(this.contracts.rebasing, this.accounts.bruce, assets);
        ...
        it('redeem', async function () {
          ...
          await expect(
            this.contracts.rebasing
              .connect(this.accounts.alice)
              .redeem(shares, this.accounts.bruce, this.accounts.alice)
          )
            .to.emit(this.contracts.rebasing, 'Transfer')
            .withArgs(this.accounts.alice, ethers.ZeroAddress, assets)
            .to.emit(this.contracts.rebasing, 'Withdraw')
